...
const EditProfile: FC = () => {
  ...
  const {
    ...
    defaultValues: {
      ...
      twitter:
        profile.loading || !profile.profile?.social?.twitter
          ? ''
          : profile.profile.social.twitter,
      facebook:
        profile.loading || !profile.profile?.social?.facebook
          ? ''
          : profile.profile.social.facebook,
      linkedin:
        profile.loading || !profile.profile?.social?.linkedin
          ? ''
          : profile.profile.social.linkedin,
      youtube:
        profile.loading || !profile.profile?.social?.youtube
          ? ''
          : profile.profile.social.youtube,
      instagram:
        profile.loading || !profile.profile?.social?.instagram
          ? ''
  ...
  const onSubmit: SubmitHandler<CreateProfileValue> = (formData) => {
    ...
    dispatch(createOrUpdateProfileAsync({ formData, history, edit: true }));
  };
